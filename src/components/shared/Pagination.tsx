...
export function Pagination({ currentPage, totalPages, baseUrl, lang }: PaginationProps) {
  ...
  const endPageInitial = Math.min(totalPages, startPage + maxVisible - 1)

  let endPage = endPageInitial
  if (endPageInitial - startPage < maxVisible - 1) {
    startPage = Math.max(1, endPageInitial - maxVisible + 1)
