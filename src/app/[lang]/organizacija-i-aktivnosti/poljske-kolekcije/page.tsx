...
export async function generateMetadata({
  ...
  const section = config.poljskeKolekcijeSection;
  return generatePageMetadata(
    siteSettings,
    section.title || 'Poljske kolekcije',
    section.shortDescription || undefined,
  );
}

export default async function PoljskeKolekcijePage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  ...
  const section = pageConfig.poljskeKolekcijeSection;
  const navItems = getOrgActivitiesNavItems(pageConfig, lang);

  return (
    <OrgActivitiesSubPage
      lang={lang}
      title={section.title || 'Poljske kolekcije'}
      description={section.shortDescription}
      breadcrumbLabel={section.title || 'Poljske kolekcije'}
